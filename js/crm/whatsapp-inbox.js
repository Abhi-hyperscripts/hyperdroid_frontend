...
(function () {
    ...
    // Heuristic: WhatsApp digits-only with country code. Detect 91 (India), 1
    // (US/Canada), 44 (UK), 971 (UAE) — fall back to first 2 digits.
    function guessCountryCode(digits) {
        if (!digits) return '';
        if (digits.startsWith('91') && digits.length === 12) return '91';
        if (digits.startsWith('1')  && digits.length === 11) return '1';
        if (digits.startsWith('44') && digits.length >= 11)  return '44';
