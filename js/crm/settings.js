...
function openWipeModal(mode) {
    ...
    if (mode === 'leads') {
        ...
        //
        // And the first correction OVERSHOT: "WhatsApp conversations are
        // deleted" is broader than what happens. whatsapp_ai_conversations and
        // whatsapp_ai_drafts are keyed by (tenant, business_phone,
        // customer_phone) with NO lead_id, so no per-lead predicate can reach
        // them — they hold customer_phone, detected_name and an LLM summary of
        // that same thread, and they survive. On an erasure dialog, claiming
        // more is destroyed than is destroyed is its own defect.
        descEl.innerHTML = `This will delete <strong>leads created between ${startDate} and ${endDate}</strong> (inclusive on both ends), plus all their lead-scoped child rows — including <strong>call records, recordings, transcripts and call scores</strong>, sequence enrolments, <strong>catalogue portal logins and the inquiries those clients submitted</strong>, <strong>uploaded documents, signature requests and the signature images on them</strong>, appointments, <strong>WhatsApp message history</strong>, activities, follow-ups, email sends (and their events/replies), tasks, notes, transfer/help requests, assignment history and other lead-scoped records. <strong>Companies, contacts, and deals are NOT deleted</strong> — they can belong to leads outside this range. The WhatsApp message history attached to these leads is deleted with them, though the AI conversation state and its drafts are keyed by phone number rather than by lead and are not reached here; only the bulk-campaign recipient rows are scrubbed rather than deleted and are not in that count.`;
    } else {
