...
window.ACCOUNTS_HELP = {
    ...
    'banking:bank-transactions':
        `<p><strong>The register of money in and out of one bank account.</strong> Most rows appear automatically from payments; use <strong>Record Transaction</strong> for things that bypass documents — bank charges, interest, owner deposits, and everyday <strong>petty-cash spends</strong>. Every manual row needs a <strong>counter account</strong>: the "other side" that explains WHY money moved. For daily spends you don't need to know account codes — pick a <strong>Spend Category</strong> (Groceries, Milk &amp; Water, Stationery, Office Boy Wages…) and the account + description fill themselves. Manage the category list in Expenses → Categories.</p>
         <p><strong>Example (petty cash):</strong> keep a "Petty Cash Box" account (type Petty Cash), top it up with an Inter-Bank Transfer of ₹10,000, then record each spend here: pick category <em>Milk &amp; Water</em>, amount ₹380 — done. Month-end, the box's book balance should match the physical cash; the P&L shows groceries, stationery and wages in their own lines.</p>`,
    ...
    'parties:customer-list':
        `<p><strong>Your customer master.</strong> State + GST registration drive invoice tax (intra vs inter-state, or zero-rated for overseas). A customer's full money story lives in Receivables; this is just who they are.</p>
         <p><strong>Example:</strong> add "Opro Maldives" with treatment <em>Overseas</em> → every invoice to them is automatically zero-rated export, no GST. Add a Bengaluru client → IGST. Set it right once, never think about it again.</p>`,
