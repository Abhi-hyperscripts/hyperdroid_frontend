...
window.ACCOUNTS_HELP = {
    ...
    // ── Inventory ─────────────────────────────────────────────────────────
    'inventory:inv-items':
        `<p><strong>Your product catalog.</strong> An <strong>item</strong> is anything you sell or buy repeatedly — a product (goods) or a service. Give it a SKU (your short code), price, GST slab and HSN code once, and billing becomes "pick item, quantity 2" instead of retyping everything. Tick <strong>Track stock</strong> for physical goods so the system counts what's on the shelf; tick <strong>Serial-tracked</strong> for things with warranties (phones, laptops) so each unit is tracked individually. The ⚙ button opens the <strong>Bill of Materials</strong> — a recipe for items you assemble from components. For pharmacies, the <strong>Composition</strong> field links the item to its salts/molecules (e.g. Paracetamol 500mg) — that's what powers <strong>substitute lookup</strong>: when a brand is out of stock at the POS, one tap shows every same-salt alternative you DO have on the shelf, exact-composition generics first.</p>
         <p><strong>Sellable vs Purchasable — where the item shows up.</strong> Two tick-boxes on each item control which screens list it. <strong>Sellable</strong> (on by default) means the item appears in the <strong>POS</strong> and on <strong>customer invoices</strong>; untick it for things you buy or make but never sell over the counter — raw materials, packaging, work-in-progress — and they vanish from every sales screen while still flowing through purchases and manufacturing. <strong>Purchasable</strong> (on by default) means the item shows when you raise a <strong>purchase order</strong> or <strong>vendor bill</strong>; untick it for products you manufacture in-house and never buy. So: raw materials = purchasable only, finished goods = sellable only, traded goods = both, services = sellable only. Hidden items carry a small <strong>Not sold</strong> / <strong>Not bought</strong> badge here in the list, and the filter chips above the table (<em>All · Sellable · Not sold · Purchasable · Not bought</em>) slice the catalog instantly — e.g. tap <strong>Not sold</strong> to see just your raw materials.</p>
    ...
    'inventory:inv-locations':
        `<p><strong>Where your stock physically sits.</strong> Add every warehouse, shop and branch, and the system tracks quantity <em>per location</em> instead of one lumped total. <strong>Transfer Stock</strong> moves units between them — for batch/lot items it carries the lot &amp; expiry along, so the destination still dispenses oldest-first and stays recall-traceable. The "Stock by location" table underneath shows exactly what's where.</p>
         <p><strong>Example:</strong> a pharmacy chain keeps a Main Warehouse and two shops. Transfer 90 strips of Amoxicillin (lot AMX-B, exp Oct 2026) to the Andheri shop; the lot moves with its expiry, the warehouse count drops by 90, and the shop can now sell it FEFO.</p>`,
    'inventory:inv-batches':
        `<p><strong>Lot &amp; expiry control for pharma, food &amp; chemicals.</strong> Batch-tracked items are received as lots (batch number + expiry), and sales dispense <strong>earliest-expiry-first (FEFO)</strong> automatically — and refuse to sell an expired lot. Three views: <em>Lot Balances</em> (what's on hand, per lot, per location), <em>Expiry Report</em> (what's about to expire and its value at risk), and the statutory <em>Schedule-H Register</em> (every dispense of an H / H1 / X controlled drug, with buyer &amp; invoice).</p>
         <p><strong>Example:</strong> two Paracetamol lots — PARA-B expires Sep 2026, PARA-A expires Jan 2027. Sell 60 strips → the system takes PARA-B first. The Expiry Report flags PARA-B as "33 days left · ₹1,376 at risk" so you can push it before it's a write-off.</p>`,
    'inventory:inv-workorders':
        `<p><strong>Manufacturing on the books.</strong> A work order turns raw materials into a finished good in two GL-clean steps: <strong>Issue</strong> moves component cost into Work-in-Progress (Dr WIP / Cr Inventory), then <strong>Complete</strong> receives the finished item at that exact cost (Dr Inventory / Cr WIP) — WIP always nets back to zero. Cancel returns issued materials. Define the recipe once via the item's Bill of Materials (⚙ on the Items tab).</p>
         <p><strong>Example:</strong> WO for 20 sub-assemblies consumes ₹5,200 of steel &amp; plastic → ₹5,200 sits in WIP while on the floor → on completion, 20 finished frames worth ₹5,200 land in stock and WIP returns to ₹0. Your books show the value moving, never appearing or vanishing.</p>`,
    'inventory:inv-count':
        `<p><strong>Physical stock-take, reconciled to the rupee.</strong> Start a count (optionally for one location), enter the quantity you actually counted for each item, and <strong>Finalize</strong> posts the difference as a GL-tied adjustment: a surplus is booked as recovery, a shortfall as shrinkage — no manual journal, and the stock-vs-GL tie is preserved. Cancel a count to discard it without touching stock.</p>
         <p><strong>Example:</strong> the system says 60 units of WAV-01 but you physically count 58. Finalize → 2 units (₹633) post as shrinkage to Cost of Goods Sold, stock drops to 58, and "Books in Sync ✓" still holds.</p>`,
    'inventory:inv-pricelists':
        `<p><strong>Different prices for different customers.</strong> Create named price lists — Wholesale, Retail/MRP, Distributor — and set a per-item price on each. Instead of one fixed sale price, you keep tiered pricing in one place and apply the right list at billing time.</p>
         <p><strong>Example:</strong> Gadget FIFO retails at ₹800, but your Wholesale list prices it at ₹720 and Distributor at ₹680. Bill a wholesaler and the ₹720 flows onto the invoice automatically — no manual discounting, no leakage.</p>`,
    'inventory:inv-schemes':
        `<p><strong>Free-goods offers that apply themselves.</strong> A scheme is a standing offer like <em>"buy 10, get 1 free"</em> — pick the item, how many BASE units the customer must buy, and how many come free (the free item can be the same product or a different one). While a scheme is active (and inside its date window, if you set one), billing does the rest: the moment an invoice line or a POS cart crosses the qualifying quantity, the free goods appear as their own line at 100% discount — stock leaves the shelf, revenue stays honest at zero for the free units, and GST is computed on what's actually charged. The biller can remove the free line for one sale without touching the scheme.</p>
         <p><strong>Example:</strong> "Monsoon 10+1" on Parle-G (buy 10 pcs, 1 free). A counter sale of 10 packs instantly shows an extra line — <em>FREE ×1 · Monsoon 10+1 · ₹0.00</em>. Ring 20 and it becomes ×2. Drop back to 9 and it disappears. Your stock report shows 11 packs gone; your sales report shows revenue for exactly 10.</p>`,
    'inventory:inv-reorder':
        `<p><strong>What to buy, before you run out.</strong> Lists every item at or below its <em>reorder level</em>, with the suggested order quantity and the preferred vendor — so a purchase run is a glance, not a guess. Set an item's reorder level and quantity on the Items tab.</p>
         <p><strong>Example:</strong> Paracetamol's reorder level is 20 strips and it's down to 18 → it appears here with "Suggested order: 100" against your usual supplier. Raise the PO before the shelf goes empty.</p>`,
    ...
    'receivables:delivery-challans':
        `<p><strong>A delivery challan moves goods when there's no invoice yet</strong> — delivery against an order, goods sent on approval ("keep what sells"), or material out for job work. Under GST Rule 55 the truck must carry a numbered document, and that's this. A challan has <strong>NO effect on stock or your books</strong>: it's transport paperwork. Flow: <strong>Draft</strong> (editable) → <strong>Issue</strong> (assigns the serial challan number so goods can move) → <strong>Convert to Invoice</strong> when the customer accepts — that creates a normal draft invoice, and only approving <em>that</em> bills the customer and deducts stock, exactly once. If the goods come back instead, just <strong>Cancel</strong> the challan.</p>
         <p><strong>Example:</strong> Palm Cove Resorts wants to see 10 water purifiers before committing. Issue challan CHN-2026-0003 (purpose: sale on approval, vehicle MH12AB1234) and send the truck. They keep 10 → Convert to Invoice → approve → ₹1,50,000 billed and stock drops by 10. Had they returned everything, you'd cancel the challan and your stock and books would be untouched — because nothing ever moved in the system.</p>`,
    'receivables:sales-orders':
        `<p><strong>Book the order now, bill it when you fulfil.</strong> A sales order captures a field or phone order — who wants what, at what agreed price, expected when, and which salesman booked it. It touches <em>nothing</em> in your books: no stock, no ledger. Flow: <strong>Draft</strong> (editable) → <strong>Confirm</strong> (assigns the serial SO number — your commitment register) → <strong>Convert to Invoice</strong> when the goods go out; the draft invoice carries the booked prices and units, and approving it bills the customer and moves stock exactly once. Customer backed out? <strong>Cancel</strong> — nothing to unwind.</p>
         <p><strong>Example:</strong> your salesman on Route 4 phones in: Acme Pvt Ltd wants 50 strips at ₹110 (agreed, below the ₹120 list), delivery Thursday. Book it in 30 seconds. Thursday morning: Convert to Invoice → the ₹110 price is already on every line → approve → billed, stock down 500 pcs, and the order shows Invoiced with a link to its bill.</p>`,
    ...
    'receivables:overdue-interest':
        `<p><strong>Late payers cost you money — this makes it visible, and chargeable.</strong> Set an annual interest rate on a customer's profile (Parties → customer → "Interest on Overdue") and every invoice they let slip past its due date starts accruing <strong>simple interest</strong>: balance × rate% × days overdue ÷ 365. This tab is the running meter. <strong>Raise Interest Invoice</strong> turns the accrued amount into a normal <em>draft</em> invoice (one line per overdue invoice) — you review, edit or drop lines, and approve it like any other invoice; nothing posts automatically. Note: raising twice for overlapping periods double-charges — the drafts record their "as of" date in each line, so check before re-raising. GST on interest for delayed payment can apply (Section 15(2)(d)) — confirm treatment with your CA before approving.</p>
         <p><strong>Example:</strong> Acme Pvt Ltd owes ₹1,00,000, now 73 days past due, at 18% p.a. → interest = 1,00,000 × 18% × 73/365 = <strong>₹3,600</strong>. Raise the invoice, and the draft carries the line "Interest @ 18% p.a. on INV-2026-0042 — 73 days overdue on ₹1,00,000". Half your late payers will suddenly find your invoices worth paying on time.</p>`,
    ...
    'banking:pdc-cheques':
        `<p><strong>The cheque drawer, on the books.</strong> A post-dated cheque (PDC) is money that <em>exists but hasn't moved</em> — a customer's cheque in your drawer, or your cheque in a vendor's hands. Register every cheque here and nothing touches your accounts until reality catches up: <strong>Deposit</strong> = handed to the bank (in transit), <strong>Clear</strong> = the bank honoured it — <em>this</em> is the moment money posts. A cleared customer cheque becomes an advance you then allocate to invoices; a cleared vendor cheque pays the bills you pick. <strong>Bounce</strong> records a dishonour (nothing had moved, so nothing reverses). For cheques you write, the <strong>print</strong> button overlays payee, date, amount in words (lakh/crore) and figures onto the bank's cheque leaf.</p>
         <p><strong>Example:</strong> Acme Pvt Ltd hands you three cheques of ₹50,000 dated the 5th of the next three months. Register all three — the "maturing in 7 days" card reminds you to deposit each on time. When the first clears, ₹50,000 lands in bank as Acme's advance; allocate it to their oldest invoice. If the second bounces, mark it — the register shows the dishonour trail your CA (and the s.138 notice) will need.</p>`,
