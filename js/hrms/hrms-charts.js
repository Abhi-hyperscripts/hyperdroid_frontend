// ============================================================================
// HRMS CHARTS — shared ApexCharts helpers (theme-aware).
// Forked from js/accounts/accounts-charts.js with three changes so it stands
// alone in HRMS:
//   1. no AccountsCommon dependency — the default formatter is plain numbers
//      (HRMS charts counts / days / hours / percentages, not money)
//   2. every builder takes a trailing `fmt` formatter, including the ones that
//      were currency-locked (acBarH / acColumns / acArea / acAreas / acLines)
//   3. empty-state class is .hrms-chart-empty
// Load AFTER apexcharts and BEFORE the page script.
// ============================================================================
const _acCharts = {};
...
if (!window._hcThemeWatched) {
    window._hcThemeWatched = true;
    try {
        new MutationObserver(() => { if (typeof _acActiveRender === 'function') _acActiveRender(); })
            .observe(document.documentElement, { attributes: true, attributeFilter: ['data-theme'] });
    } catch (e) { /* no-op */ }
}
// Self-contained default formatter — HRMS mostly charts counts/days/hours, so
// the default is a plain number; money charts pass their own `fmt` (payroll is
// multi-currency via currency_code/currency_symbol on the run/payslip).
const _inr = (v) => {
...
function _acEmpty(id, msg) {
    ...
    if (!el) return;
    if (_acCharts[id]) { _acCharts[id].destroy(); delete _acCharts[id]; }
    el.innerHTML = `<div class="hrms-chart-empty">${msg || 'No data for this period'}</div>`;
}
...
function acBarH(id, labels, data, fmt) {
    if (!data.length) return _acEmpty(id);
    const t = _acTheme();
    const maxV = Math.max(...data);
    const f = fmt || _inr;
    const fc = fmt || _inrCompact;
    _acMount(id, {
        ...
        dataLabels: { enabled: true, formatter: (v) => v >= maxV * 0.18 ? fc(v) : '', style: { fontSize: '11px', fontWeight: 600, colors: ['#fff'] } },
        xaxis: { categories: labels, tickAmount: 4, labels: { formatter: fc, style: { colors: t.text, fontSize: '11px' } }, axisBorder: { show: false }, axisTicks: { show: false } },
...
function acColumns(id, categories, series, colors, fmt) {
    ...
    _acMount(id, {
        ...
        grid: { borderColor: t.grid, strokeDashArray: 4 },
        tooltip: { theme: t.isDark ? 'dark' : 'light', shared: true, intersect: false, y: { formatter: f } }
    });
}
function acArea(id, categories, data, name, fmt) {
    if (!data.length || data.every(v => !v)) return _acEmpty(id);
    const t = _acTheme();
    const f = fmt || _inr;
    const fc = fmt || _inrCompact;
...
function acAreas(id, categories, series, colors, height = 320, fmt) {
    if (!series || !series.length || series.every(s => (s.data || []).every(v => !v))) return _acEmpty(id);
    const t = _acTheme();
    const f = fmt || _inr;
    const fc = fmt || _inrCompact;
    _acMount(id, {
        ...
        yaxis: { labels: { formatter: fc, style: { colors: t.text, fontSize: '11px' } } },
        ...
        tooltip: { theme: t.isDark ? 'dark' : 'light', shared: true, y: { formatter: f } }
    });
}
function acLines(id, categories, series, colors, fmt) {
    if (!series || !series.length || series.every(s => (s.data || []).every(v => !v))) return _acEmpty(id);
    const t = _acTheme();
    _acMount(id, {
