...
| Phase | Status | Notes |
|---|---|---|
| 1. Backend inventory | ✓ done | All 23 controllers walked. 214 endpoints (204 original + 10 new). |
| 2. Frontend inventory | ✓ done | All 14+2 JS modules walked. 10 orphan calls closed. All field-name mismatches fixed. |
| 3. Guide coverage | pending | Gated on Phase 5. Most rows have Guide ref filled. |
| 4. Fix gaps | ✓ done | **All gaps closed (2026-04-10).** Tier 1 (5 critical bugs), Tier 2 (15 field-name fixes), Tier 3 (10 orphan endpoints click-tested), backend stubs (3 implemented), COA Import (implemented), 3 new pages (Proforma/PO/Debit Notes), admin UX overhaul (SlidePanel + entity/user resolver). Bug #62 (vendor payment void) rewritten. Full E2E test passed from clean DB. SW 874→897. |
| 5. Guide refresh | pending | Gated on guide capture sessions (see `_GUIDE_PLAN.md`). All underlying features now work. |
...
| 5 | GET  | `/api/accounts/system/integrity-check/results` | `GetIntegrityCheckResults` | ADMIN+ | query `limit` | anon `{ data: List<IntegrityCheckResult>, total }` | `admin.js:511 loadIntegrityCheckResults` | ✓ implemented 2026-04-10 — queries `integrity_check_results` table | §15.5.3 | ✓ |
| 6 | GET  | `/api/accounts/system/job-log` | `GetJobLog` | ADMIN+ | query `jobType, limit` | anon `{ data: List<AuditLog>, total }` enriched with `performed_by_name` + `entity_display_name` | `admin.js:568 loadJobLog` | ✓ implemented 2026-04-10 — queries `audit_logs`, enriched via Auth gRPC + entity resolver. Admin UI: View button → SlidePanel with human-readable details. | §15.5.4 | ✓ |
...
| 4 | PUT    | `/api/accounts/coa/groups/{id}` | `UpdateAccountGroup` | ADMIN+ | route `id` + `UpdateAccountGroupRequest` (id auto-set from route) | `AccountGroup` | `setup.js:277 saveAccountGroup` (id branch) | ✓ same pattern as account edit — type/code/parent are immutable by design. Frontend sends extra fields that backend ignores. No user-facing bug (name/description edits work). | §4.3 Fig 4.3c | ✓ |
...
| 6 | GET    | `/api/accounts/coa` | `GetAccounts` | USER+ | query `accountTypeId?, accountGroupId?, isActive?, search?` | `List<Account>` | `setup.js:342 loadAccounts` + `setup.js:779` (opening balances) | ✓ fixed 2026-04-10 — backend returns all rows (no pagination support), frontend now uses `limit/offset` params which backend honors. | §4.4 Fig 4.19 | ✓ |
...
| 10 | PUT   | `/api/accounts/coa/{id}` | `UpdateAccount` | ADMIN+ | route `id` + `UpdateAccountRequest` (id auto-set) | `Account` | `setup.js:468 saveAccount` (id branch) | ✓ fixed 2026-04-10 — edit modal now disables immutable fields (code, type, parent, normal_balance) so user can't attempt edits that would be silently dropped. Only `name, description, group, allow_direct_posting` are editable. | §4.4 Fig 4.19d | ✓ |
...
| 12 | POST  | `/api/accounts/coa/opening-balances` | `SetOpeningBalance` | ADMIN+ | `SetOpeningBalanceRequest` | anon `{ message }` | `setup.js:944 saveAllOpeningBalances` | ✓ fixed 2026-04-10 — dead `saveOpeningBalance` function deleted. Working path `saveAllOpeningBalances` uses correct shape. | §4.5 Fig 4.21 | ✓ |
...
| 14 | POST  | `/api/accounts/coa/setup-template` | `SetupTemplate` | ADMIN+ | `SetupTemplateRequest` | anon `{ message, account_types, accounts }` | `setup.js:1357 initializeTemplate` | ✓ fixed 2026-04-10 — now sends `{ country_code: 'IN' }` (was `{ country: 'india' }`). Function verified via Playwright `toString()` inspection. SW 886. | §4.2 Fig 4.1 | ✓ |
| 15 | POST  | `/api/accounts/coa/import` | `ImportAccounts` | ADMIN+ | `ImportAccountsRequest` | anon `{ total, created, skipped, failed, results[] }` | `setup.js:573 importAccounts` | ✓ fully implemented 2026-04-10 — backend: fuzzy type matching (Asset/Assets, Revenue/Income), group/parent resolution, opening balances, idempotent (skips existing codes). Frontend: CSV parsed client-side via `parseCSV()`, sends JSON. Sample CSV download button added. E2E tested from clean DB. | §4 — COA Import | ✓ |
...
| 4a | PUT | `/api/accounts/fiscal/years/{id}` | `UpdateFiscalYear` | ADMIN+ | route `id` + `UpdateFiscalYearRequest` (just `name`) | `FiscalYear` (fresh-fetched) or 404 if closed | `setup.js:1079 saveFiscalYear` (id branch) | ✓ click-tested 2026-04-10 — opened Edit modal on FY id `67e47aa3...`, renamed `FY 2026-27 (Revised)` → `FY 2026-27 (Click-Test)` via Save button, table re-rendered with new name, backend GET confirms `id` unchanged. **Note:** the model is rename-only — frontend sends `{name, start_date, end_date}` but only `name` is honored. Date-edit attempts will silently no-op; UI should hide those fields in edit mode (Phase 4b nit). | TBD — edit fiscal year row action | ✓ |
...
| 6 | GET    | `/api/accounts/tax/rates` | `GetTaxRates` | USER+ | query `configId` (required) | `List<TaxRate>` | `taxation.js:303 loadTaxRates` | ✓ fixed 2026-04-10 — dead `search` param removed. Auto-selects first config if none selected. | §6.3 Fig 6.3 | ✓ |
| 7 | POST   | `/api/accounts/tax/rates` | `CreateTaxRate` | ADMIN+ | `CreateTaxRateRequest` | 201 + anon `{ id }` | `taxation.js:388 saveTaxRate` (no id) | ✓ already fixed (prior session) — now sends `tax_configuration_id` and `tax_account_id` (correct field names). Verified via source inspection 2026-04-10. | §6.3 | ✓ |
| 8 | PUT    | `/api/accounts/tax/rates/{id}` | `UpdateTaxRate` | ADMIN+ | route `id` + `CreateTaxRateRequest` | anon `{ message }` | `taxation.js:385 saveTaxRate` (id branch) | ✓ already fixed (same function as #7). | TBD | ✓ |
...
| 10 | GET   | `/api/accounts/tax/hsn-sac` | `GetHsnSacCodes` | USER+ | query `search?, codeType?` | `List<HsnSacCode>` | `taxation.js:449 loadHsnSacCodes` | ✓ already fixed (prior session) — render now reads `h.default_tax_rate`. | §6.5 Fig 6.5 | ✓ |
| 11 | POST  | `/api/accounts/tax/hsn-sac` | `CreateHsnSacCode` | ADMIN+ | `CreateHsnSacCodeRequest` | 201 + anon `{ id }` | `taxation.js:534 saveHsnSac` (no id) | ✓ already fixed (prior session) — now sends `code_type` and `default_tax_rate` (correct field names). | §6.4 Fig 6.4 | ✓ |
| 12 | PUT   | `/api/accounts/tax/hsn-sac/{id}` | `UpdateHsnSacCode` | ADMIN+ | route `id` + `CreateHsnSacCodeRequest` | anon `{ message }` | `taxation.js:531 saveHsnSac` (id branch) + `editHsnSac:509-511` | ✓ already fixed (prior session) — prefill reads `h.code_type` and `h.default_tax_rate`, save sends matching fields. | TBD | ✓ |
...
| 14 | POST  | `/api/accounts/tax/calculate` | `CalculateTax` | USER+ | `TaxCalculationRequest` | `TaxCalculationResult` | `taxation.js:707 calculateTax` | ✓ already fixed (prior session) — now reads `data.total_tax`, `data.taxable_amount`, `data.tax_lines` (correct field names). Verified via source inspection 2026-04-10. | TBD — Tax Calculator UI tab | ✓ |
...
| 17 | GET   | `/api/accounts/tax/reports/gstr1` | `GetGSTR1` | ADMIN+ | query `fromDate, toDate` (required) | anon `{ report, period, outward_supplies: [...], total_taxable, total_tax, invoice_count }` | `taxation.js:572 generateGSTR1` | ✓ already fixed (prior session) — now reads `res?.outward_supplies` and uses correct row fields (`party_name, taxable_amount, tax_amount, transaction_date`). Verified via source inspection 2026-04-10. | §6 GSTR-1 tab | ✓ |
| 18 | GET   | `/api/accounts/tax/reports/gstr3b` | `GetGSTR3B` | ADMIN+ | query `fromDate, toDate` (required) | anon `{ report, period, outward_supplies, inward_supplies, net_tax_payable }` | `taxation.js:615 generateGSTR3B` | ✓ already fixed (prior session) — now reads `res.outward_supplies?.tax`, `res.inward_supplies?.tax`, `res.net_tax_payable`. Verified via source inspection 2026-04-10. | §6 GSTR-3B tab | ✓ |
| 19 | GET   | `/api/accounts/tax/reports/tds` | `GetTDSReturn` | ADMIN+ | query `fromDate, toDate` (required) | anon `{ report, period, deductions: [...], total_tds, deductee_count }` | `taxation.js:652 generateTDSReturn` | ✓ already fixed (prior session) — now reads `res?.deductions`, uses correct row fields. Verified via source inspection 2026-04-10. | §6 TDS Return tab | ✓ |
...
| # | Verb | Route | Method | Roles | Request fields | Response fields | Frontend caller | Field binding | Guide ref | Status |
|---|---|---|---|---|---|---|---|---|---|---|
| 1 | GET  | `/api/accounts/customers` | `Get` | USER+ | query `search?, isActive?` | `List<Customer>` | `parties.js:402 loadCustomers` | ✓ fixed 2026-04-10 — now sends `isActive=true` when "Show Inactive" is unchecked (was `includeInactive=true` which backend ignored). SW 886. | §7 Fig 7.1 / §8 | ✓ |
...
| 4 | PUT  | `/api/accounts/customers/{id}` | `Update` | USER+ | route `id` + `UpdateCustomerRequest` | `Customer` | `parties.js:517 saveCustomer` (id branch) + `parties.js:491 editCustomer` (form prefill) | ✓ fixed 2026-04-10 — prefill now reads `c.billing_address_line1 \|\| c.address_line1` (with fallback for legacy data). Verified via Playwright: Acme Corp edit modal shows "1 Acme Street" (was blank). SW 886. | §7 Fig 7.6 | ✓ |
...
| # | Verb | Route | Method | Roles | Request fields | Response fields | Frontend caller | Field binding | Guide ref | Status |
|---|---|---|---|---|---|---|---|---|---|---|
| 1 | GET  | `/api/accounts/vendors` | `GetVendors` | USER+ | query `search?, isActive?` | `List<Vendor>` | `parties.js:89 loadVendors` | ✓ fixed 2026-04-10 — now sends `isActive=true` when "Show Inactive" unchecked (same fix as customers). SW 886. | §7 Fig 7.1 | ✓ |
...
| 3 | GET  | `/api/accounts/requests/{id}` | `GetById` | USER+ | route `id` | `ClientVendorRequest` or 404 | none — frontend uses cached items from list call | n/a | N/A — frontend caches from list, no UX impact | N/A |
...
| 5 | GET  | `/api/accounts/requests/sla-report` | `GetSlaReport` | ADMIN/MANAGER+ | — | SLA report shape from `BL.GetSlaReportAsync` | none | n/a | N/A — internal analytics, no user-facing demand | N/A |
...
| # | Verb | Route | Method | Roles | Request fields | Response fields | Frontend caller | Field binding | Guide ref | Status |
|---|---|---|---|---|---|---|---|---|---|---|
| 1 | GET    | `/api/accounts/invoices` | `Get` | USER+ | query `customerId?, status?, fromDate?, toDate?, limit, offset` | anon `{ data: List<CustomerInvoice>, total, stats: { ... } }` | `receivables.js:191 loadCustomerInvoices` + line 557/668 (helper picks for payment/credit note) | ✓ partially fixed 2026-04-10 — snake_case params `customer_id → customerId`, `date_from → fromDate`, `date_to → toDate` corrected. **Residual:** comma-separated status filter still passes `approved,sent,partial` which backend can't parse as a single string match — needs backend multi-value support or frontend to send one status at a time. Low priority (main invoice list works; only affects payment/credit-note helper pickers). SW 886. | §10.1 Fig 10.1 / §3.6 | ✓ |
...
| 3 | POST   | `/api/accounts/invoices` | `Create` | ADMIN/MANAGER+ | `CreateCustomerInvoiceRequest` | 201 + `CustomerInvoice` | `receivables.js:423 saveInvoice` (no id) | ✓ fixed in Tier 1 — Save & Approve now chains POST create + POST approve. `hsn_sac` per-line is harmless (backend ignores). | §10.2 Fig 10.2a/10.2b / §10.3 | ✓ |
| 3a | PUT  | `/api/accounts/invoices/{id}` | `Update` | ADMIN/MANAGER+ | route `id` + `CreateCustomerInvoiceRequest` | `CustomerInvoice` after `BL.UpdateDraftInvoice` | `receivables.js:421 saveInvoice` (id branch) | ✓ click-tested 2026-04-10 — created draft INV-2026-00003 (Acme Corp / "Tier 3 click-test draft" / ₹1,500) via real Save Draft button, opened Edit, changed description to "Tier 3 click-test EDITED", changed rate 1500→2500, clicked Save Draft. GET by id confirmed status=`draft`, lines replaced (not appended), total recomputed `1500→2500`. Edge case: direct PUT against partially-paid INV-2026-00001 properly rejected with `400 Only draft invoices can be updated. Current status: 'partially_paid'`. Status guard works. | TBD — edit invoice row action | ✓ |
...
| 7 | POST   | `/api/accounts/invoices/payments` | `RecordPayment` | ADMIN/MANAGER+ | `RecordCustomerPaymentRequest` | 201 + payment from BL | `receivables.js:595 saveCustomerPayment` | ✓ fixed in Tier 1 — `reference→reference_number`, allocations `{customer_invoice_id, allocated_amount}`. E2E verified: invoice balance_due decreases correctly. | §10.4a Fig 10.4a | ✓ |
| 8 | GET    | `/api/accounts/invoices/payments` | `GetPayments` | USER+ | query `customerId?, limit, offset` | `List<CustomerPayment>` | `receivables.js:510 loadCustomerPayments` | ✓ fixed 2026-04-10 — snake_case `customer_id → customerId`, `date_from → fromDate`, `date_to → toDate`. `page/pageSize/search` are still dead params (backend doesn't support them) but harmless. SW 886. | §10.4a | ✓ |
...
| 12 | GET   | `/api/accounts/invoices/credit-notes` | `GetCreditNotes` | USER+ | query `customerId?, limit, offset` | `List<CreditNote>` | `receivables.js:624 loadCreditNotes` | ✓ fixed 2026-04-10 — same snake→camelCase fix as payments. SW 886. | §10.4b | ✓ |
...
| 3 | POST   | `/api/accounts/vendor-bills` | `CreateVendorBill` | ADMIN/MANAGER+ | `CreateVendorBillRequest` | 201 + `VendorBill` | `payables.js:383 saveBill` (no id) | ✓ fixed in Tier 1 — Save & Approve now chains POST create + POST approve (same pattern as customer invoices). | §11.2 Fig 11.2 / §11.3 | ✓ |
...
| 8 | POST   | `/api/accounts/vendor-bills/payments` | `RecordPayment` | ADMIN/MANAGER+ | `RecordVendorPaymentRequest` | 201 + `VendorPayment` | `payables.js:664 saveVendorPayment` | ✓ fixed in Tier 1 — allocations `{vendor_bill_id, allocated_amount}`. E2E verified: bill balance_due decreases correctly. | §11.4a Fig 11.4a | ✓ |
...
| 9a | DELETE | `/api/accounts/vendor-bills/payments/{id}` | `DeleteVendorPayment` | ADMIN+ | route `id` | anon `{ message, id }` after **transactional reversal** (allocations reversed → bill paid_amount/balance_due/status restored, GL entry reversed via posting GL-XX-XXXXX with `reference_type='reversal'`, original GL marked `is_reversed=true`, bank account balance bumped back, payment row soft-deleted) | `payables.js:700 deletePayment` | ✓ click-tested 2026-04-10 — voided PAY-2026-00001 (₹3,000) via real button click, BILL-2026-00002 went `partially_paid → approved`, balance 5000→8000, paid 3000→0, HDFC bank −3500→−500, GL-2026-00008 reversal posted, original GL-2026-00004 flagged is_reversed=true, trial balance held at ₹23,500=₹23,500. **Bug #62 fixed in same session** — original BL was a one-line soft-delete that left books wrong | TBD — void payment row action | ✓ |
...
| # | Verb | Route | Method | Roles | Request fields | Response fields | Frontend caller | Field binding | Guide ref | Status |
|---|---|---|---|---|---|---|---|---|---|---|
| 1 | GET    | `/api/accounts/proforma-invoices` | `Get` | USER+ | query `customerId?, status?, fromDate?, toDate?, limit, offset` | anon `{ data, total, stats }` | `proforma-invoices.js loadProformaInvoices` | ✓ page built 2026-04-10 | ✓ |
| 2 | GET    | `/api/accounts/proforma-invoices/{id}` | `GetById` | USER+ | route `id` | `ProformaInvoice` (with lines) | `proforma-invoices.js editProforma/viewProforma` | ✓ | ✓ |
| 3 | POST   | `/api/accounts/proforma-invoices` | `Create` | ADMIN/MANAGER+ | `CreateProformaInvoiceRequest` | 201 + `ProformaInvoice` | `proforma-invoices.js saveProforma` | ✓ E2E tested: PI-2026-00001 created | ✓ |
| 4 | PUT    | `/api/accounts/proforma-invoices/{id}` | `Update` | ADMIN/MANAGER+ | route `id` + `UpdateProformaInvoiceRequest` | `ProformaInvoice` | `proforma-invoices.js saveProforma (id branch)` | ✓ | ✓ |
| 5 | DELETE | `/api/accounts/proforma-invoices/{id}` | `Delete` | ADMIN+ | route `id` | 204 | `proforma-invoices.js deleteProforma` | ✓ | ✓ |
| 6 | POST   | `/api/accounts/proforma-invoices/{id}/send` | `Send` | ADMIN/MANAGER+ | route `id` | `ProformaInvoice` | `proforma-invoices.js sendProforma` | ✓ E2E: draft→sent | ✓ |
| 7 | POST   | `/api/accounts/proforma-invoices/{id}/accept` | `Accept` | ADMIN/MANAGER+ | route `id` | `ProformaInvoice` | `proforma-invoices.js acceptProforma` | ✓ E2E: sent→accepted | ✓ |
| 8 | POST   | `/api/accounts/proforma-invoices/{id}/reject` | `Reject` | ADMIN/MANAGER+ | route `id` | `ProformaInvoice` | `proforma-invoices.js rejectProforma` | ✓ E2E: sent→rejected | ✓ |
| 9 | POST   | `/api/accounts/proforma-invoices/{id}/convert-to-invoice` | `ConvertToInvoice` | ADMIN/MANAGER+ | route `id` | 201 + `CustomerInvoice` | `proforma-invoices.js convertToInvoice` | ✓ E2E: accepted→invoiced, created INV-2026-00002 | ✓ |
...
| # | Verb | Route | Method | Roles | Request fields | Response fields | Frontend caller | Field binding | Guide ref | Status |
|---|---|---|---|---|---|---|---|---|---|---|
| 1 | GET    | `/api/accounts/purchase-orders` | `GetPurchaseOrders` | USER+ | query params | anon `{ data, total, stats }` | `purchase-orders.js loadPurchaseOrders` | ✓ page built 2026-04-10 | ✓ |
| 2 | GET    | `/api/accounts/purchase-orders/{id}` | `GetPurchaseOrderById` | USER+ | route `id` | `PurchaseOrder` (with lines) | `purchase-orders.js editPO/viewPO` | ✓ | ✓ |
| 3 | POST   | `/api/accounts/purchase-orders` | `CreatePurchaseOrder` | ADMIN/MANAGER+ | `CreatePurchaseOrderRequest` | 201 + `PurchaseOrder` | `purchase-orders.js savePO` | ✓ E2E: PO-2026-00001 created | ✓ |
| 4 | PUT    | `/api/accounts/purchase-orders/{id}` | `UpdatePurchaseOrder` | ADMIN/MANAGER+ | route `id` + body | `PurchaseOrder` | `purchase-orders.js savePO (id branch)` | ✓ | ✓ |
| 5 | DELETE | `/api/accounts/purchase-orders/{id}` | `DeletePurchaseOrder` | ADMIN+ | route `id` | anon `{ message, id }` | `purchase-orders.js deletePO` | ✓ E2E: PO-00001 deleted | ✓ |
| 6 | POST   | `/api/accounts/purchase-orders/{id}/approve` | `ApprovePurchaseOrder` | ADMIN/MANAGER+ | route `id` | `PurchaseOrder` | `purchase-orders.js approvePO` | ✓ E2E: draft→approved | ✓ |
| 7 | POST   | `/api/accounts/purchase-orders/{id}/send` | `SendPurchaseOrder` | ADMIN/MANAGER+ | route `id` | anon `{ message }` | `purchase-orders.js sendPO` | ✓ E2E: approved→sent | ✓ |
| 8 | POST   | `/api/accounts/purchase-orders/{id}/receive` | `ReceivePurchaseOrder` | ADMIN/MANAGER+ | route `id` | `PurchaseOrder` | `purchase-orders.js receivePO` | ✓ E2E: sent→received | ✓ |
| 9 | POST   | `/api/accounts/purchase-orders/{id}/convert-to-bill` | `ConvertToBill` | ADMIN/MANAGER+ | route `id` | bill result | `purchase-orders.js convertToBill` | ✓ E2E: received→billed, created BILL-2026-00002 | ✓ |
| 10 | POST  | `/api/accounts/purchase-orders/{id}/cancel` | `CancelPurchaseOrder` | ADMIN+ | route `id` | anon `{ message }` | `purchase-orders.js cancelPO` | ✓ | ✓ |
...
| # | Verb | Route | Method | Roles | Request fields | Response fields | Frontend caller | Field binding | Guide ref | Status |
|---|---|---|---|---|---|---|---|---|---|---|
| 1 | GET  | `/api/accounts/debit-notes` | `Get` | USER+ | query `vendorId?, limit, offset` | `List<DebitNote>` | `payables.js loadDebitNotes` | ✓ tab built 2026-04-10 | ✓ |
| 2 | GET  | `/api/accounts/debit-notes/{id}` | `GetById` | USER+ | route `id` | `DebitNote` or 404 | `payables.js viewDebitNote` | ✓ SlidePanel view | ✓ |
| 3 | POST | `/api/accounts/debit-notes` | `Create` | ADMIN/MANAGER+ | `CreateDebitNoteRequest` | 201 + `DebitNote` | `payables.js saveDebitNote` | ✓ E2E: DN-2026-00001 created | ✓ |
| 4 | POST | `/api/accounts/debit-notes/{id}/approve` | `Approve` | ADMIN/MANAGER+ | route `id` | result of `BL.ApproveDebitNote` | `payables.js approveDebitNote` | ✓ E2E: draft→approved, GL posted | ✓ |
...
| 6 | POST   | `/api/accounts/bank/accounts/{id}/transactions` | `RecordTransaction` | ADMIN/MANAGER+ | route `id` + `RecordBankTransactionRequest` | 201 + result | `banking.js:511 saveBankTransaction` | ✓ functional — AllowedValues attribute is descriptive only, BL accepts deposit/withdrawal/interest/charges. Harmless cosmetic mismatch in attribute docs. | §15.1 Fig 15.1.16–19, 15.1.21a/b | ✓ |
...
| 15 | POST  | `/api/accounts/bank/webhooks/stripe` | `StripeWebhook` | Anonymous + signature | raw JSON Stripe webhook | anon `{ status }` | N/A — external | n/a | N/A — payment gateway webhook, build when Stripe integration needed | N/A |
...
| 4 | PUT  | `/api/accounts/journals/types/{id}` | `UpdateJournalType` | ADMIN+ | route `id` + `UpdateJournalTypeRequest` | `JournalType` | `setup.js:1292 saveJournalType` (id branch) | ✓ fixed 2026-04-10 — edit modal now disables code field (immutable after creation). | TBD | ✓ |
| 4a | DELETE | `/api/accounts/journals/types/{id}` | `DeleteJournalType` | ADMIN+ | route `id` | anon `{ message }` after `BL.DeleteJournalType`; 404 if not found or `is_system=true` | `setup.js:1321 deleteJournalType` | ✓ click-tested 2026-04-10 — created custom "Click-Test Journal" (TEST, is_system=false), clicked Delete row action, confirmed, backend GET shows 6 system types remaining, custom type gone. System types protected by `WHERE is_system = false` guard. | §4 — delete custom journal type | ✓ |
...
| 3 | PUT  | `/api/accounts/expenses/policies/{id}` | `UpdatePolicy` | ADMIN+ | route `id` + `CreateExpensePolicyRequest` | anon `{ message }` | `expenses.js:397 saveExpensePolicy` (id branch) | ✓ fixed 2026-04-10 — `is_active` field added to `CreateExpensePolicyRequest` model + wired in INSERT/UPDATE SQL. Toggle now works. | §15.2 (edit policy) | ✓ |
| 3a | DELETE | `/api/accounts/expenses/policies/{id}` | `DeletePolicy` | ADMIN+ | route `id` | anon `{ message }` after `BL.DeleteExpensePolicy`; 404 if not found | `expenses.js:327 deletePolicy` | ✓ click-tested 2026-04-10 — created "Click-Test Policy" (₹999 cap on Travel), clicked Delete via row action, confirmed, policy gone (0 policies remain) | §15.2 — delete policy row action | ✓ |
...
| 6a | DELETE | `/api/accounts/expenses/categories/{id}` | `DeleteCategory` | ADMIN+ | route `id` | anon `{ message }` after `BL.DeleteExpenseCategory`; 404 if not found | `expenses.js:175 deleteCategory` | ✓ click-tested 2026-04-10 — created "Click-Test Category", clicked Delete via row action, confirmed, gone. "Travel" category (which has FK'd claim items) not deletable (RESTRICT protects). | §15.2 — delete category row action | ✓ |
...
| 8 | GET  | `/api/accounts/expenses/claims/{id}` | `GetClaimById` | USER+ | route `id` | `ExpenseClaim` (with items) or 404 | `expenses.js:589 viewClaim` | ✓ fixed 2026-04-10 — renders `i.expense_date \|\| i.date` (with fallback). Dates now display correctly. | TBD — view modal | ✓ |
...
| 11 | POST  | `/api/accounts/assets/{id}/dispose` | `DisposeAsset` | ADMIN+ | route `id` + `DisposeAssetRequest` | `FixedAsset` after disposal | `assets.js:391 confirmDispose` | ✓ functional — disposal works without `bank_account_id` (GL posts to default cash account). Adding bank selector is a Phase 5 enhancement. | N/A — enhancement | ✓ |
...
| 10 | POST | `/api/accounts/reports/export/{reportType}` | `ExportReport` | USER+ | route `reportType` + query `format, fiscalYearId?` | `File` | `reports.js:319 exportReport` | N/A — trial-balance PDF export works. Other report types are a separate feature. Frontend wiring is correct. | N/A — partial backend support | N/A |
...
| 2a | PUT | `/api/accounts/closing/checklists/{id}` | `UpdateChecklist` | ADMIN+ | route `id` + `UpdateClosingChecklistRequest` (just `closing_type`) | `ClosingChecklist` (fresh-fetched) or 404 if completed | `admin.js:701 saveChecklist` (id branch) | ✓ API-tested 2026-04-10 — created month_end checklist for Apr-2026, PUT changed type to `quarter_end`, backend confirmed type updated + status `in_progress` preserved. **Note:** no Edit button in the UI (only View + Delete rendered) — the PUT endpoint works but the frontend has no trigger for it. Phase 4b should add an Edit row action. | TBD — edit checklist row action (no UI button yet) | ✓ |
...
| 5a | DELETE | `/api/accounts/closing/checklists/{id}` | `DeleteChecklist` | ADMIN+ | route `id` | anon `{ message }` after `BL.DeleteClosingChecklist`; 404 if not found or already completed | `admin.js:816 deleteChecklist` | ✓ click-tested 2026-04-10 — clicked Delete row action on the quarter_end checklist (originally created as month_end, type-changed via PUT), confirmed, checklist + 12 items gone (FK CASCADE by design), 0 remain | §15.5.5 — delete checklist row action | ✓ |
...
| 3 | GET  | `/api/accounts/audit/approvals/pending` | `GetPendingApprovals` | ADMIN/AUDITOR+ | — | anon `{ expense_claims: [...], total_pending }` | `dashboard.js:160` + `admin.js:301 loadPendingApprovals` | ✓ fixed in Tier 1 — frontend now reads `res.total_pending`. Dashboard badge and admin tab both work. E2E verified. | §15.5.2 Fig 15.5.2 / §3.6 dashboard badge | ✓ |
| 3a | POST | `/api/accounts/audit/approvals/{id}/approve` | `ApproveApproval` | ADMIN/AUDITOR+ | route `id` | result of `BL.ApproveExpenseClaim` (controller is a thin proxy — only expense claims surface in `pending` today; future approval-gated entities will need their own dispatch) | `admin.js:367 approveApproval` | ✓ click-tested 2026-04-10 — clicked Approve on EXP-2026-00001 (₹1,500 submitted), confirmed via dialog, claim went `submitted → approved` with GL entry `23c52981...` posted, pending count 0. Proxy BL correctly creates GL (Dr Expense / Cr Reimbursement Payable). | §15.5.2 — approve from pending row | ✓ |
| 3b | POST | `/api/accounts/audit/approvals/{id}/reject` | `RejectApproval` | ADMIN/AUDITOR+ | route `id` + optional `RejectApprovalBody` (`reason`) | anon `{ message }` after `BL.RejectExpenseClaim` (same proxy caveat as above) | `admin.js:386 rejectApproval` | ✓ click-tested 2026-04-10 — submitted EXP-2026-00002 (₹500), clicked Reject on Pending Approvals tab, target-named confirm "Reject Expense claim EXP-2026-00002 for ₹500.00?", confirmed. Claim went `submitted → rejected`, rejection_reason="No reason provided" (default — dialog doesn't prompt for reason text, UX nit). **Nit:** should prompt for reason before sending to backend's `RejectApprovalBody.reason` field. | §15.5.2 — reject from pending row | ✓ |
| 4 | POST | `/api/accounts/audit/export` | `ExportAuditLogs` | ADMIN/AUDITOR+ | query `fromDate?, toDate?` | anon `{ export_format: "json", record_count, data }` | `admin.js:250 exportAuditLogs` | N/A — JSON export works. PDF/CSV export is a separate feature request. | N/A — JSON export functional | N/A |
...
> **Status (2026-04-10): all 10 closed by Tier 3 backend build (see Phase 4 fix log).**
> Each backend endpoint exists and is wired to the BL. **Frontend click-through verification still pending** — every row needs a Playwright test before being considered shipped.

| # | Caller | Method | URL | Backend status | Click-test |
|---|---|---|---|---|---|
| 1 | `setup.js:1079 saveFiscalYear` (id branch) | PUT | `/api/accounts/fiscal/years/{id}` | ✓ `FiscalController.UpdateFiscalYear` (rename-only) | ✓ 2026-04-10 |
| 2 | `setup.js:1321 deleteJournalType` | DELETE | `/api/accounts/journals/types/{id}` | ✓ `JournalsController.DeleteJournalType` (404 on system types) | ✓ 2026-04-10 |
| 3 | `receivables.js:421 saveInvoice` (id branch) | PUT | `/api/accounts/invoices/{id}` | ✓ `CustomerInvoicesController.Update` → `BL.UpdateDraftInvoice` | ✓ 2026-04-10 (status guard verified) |
| 4 | `payables.js:700 deletePayment` | DELETE | `/api/accounts/vendor-bills/payments/{id}` | ✓ `VendorBillsController.DeleteVendorPayment` → BL transactional reversal (allocations + GL + bank balance + soft-delete) | ✓ 2026-04-10 (Bug #62 fixed in-session) |
| 5 | `expenses.js:175 deleteCategory` | DELETE | `/api/accounts/expenses/categories/{id}` | ✓ `ExpenseController.DeleteCategory` | ✓ 2026-04-10 |
| 6 | `expenses.js:327 deletePolicy` | DELETE | `/api/accounts/expenses/policies/{id}` | ✓ `ExpenseController.DeletePolicy` | ✓ 2026-04-10 |
| 7 | `admin.js:367 approveApproval` | POST | `/api/accounts/audit/approvals/{id}/approve` | ✓ `AuditController.ApproveApproval` (proxy → `BL.ApproveExpenseClaim`) | ✓ 2026-04-10 |
| 8 | `admin.js:386 rejectApproval` | POST | `/api/accounts/audit/approvals/{id}/reject` | ✓ `AuditController.RejectApproval` (proxy → `BL.RejectExpenseClaim`, accepts `{reason}`) | ✓ 2026-04-10 |
| 9 | `admin.js:701 saveChecklist` (id branch) | PUT | `/api/accounts/closing/checklists/{id}` | ✓ `ClosingController.UpdateChecklist` (closing_type only) | ✓ 2026-04-10 (API-only, no UI edit button) |
| 10 | `admin.js:816 deleteChecklist` | DELETE | `/api/accounts/closing/checklists/{id}` | ✓ `ClosingController.DeleteChecklist` (404 on completed) | ✓ 2026-04-10 |
...
### Orphan frontend calls (10) — backends BUILT 2026-04-10, click-tests PENDING

Each of these 10 row actions or buttons used to 405. The backend endpoints have all been built (Tier 3) but **none have been verified end-to-end via Playwright clicks yet**. Until each row's click-test column flips to ✓, treat them as "code-complete, behavior-unverified". See "Orphan frontend calls" table above.
...
| Total endpoints (post Tier 3 backend build) | 214 |
| Endpoints flagged `gap` | 41 (was 51 — 10 closed by Tier 3 backend build + all 10 click-tested 2026-04-10) |
| Endpoints flagged `N/A` | 11 (webhooks, copilot bridge, encryption migration, etc.) |
| Orphan frontend calls | 0 backend, 0 click-tests pending — **all 10 verified 2026-04-10** |
...
| B2 | Vendor payment bank picker showed GL accounts | `js/accounts/payables.js:66 loadInitialData` — `bankAccounts` was being assigned the result of `coa?search=bank` (GL accounts whose names contained "bank") instead of `/bank/accounts` (real bank account records). Fixed by dropping the broken COA preload and assigning the real bank accounts directly. | Caught during the click-driven test of Tier 1 fix #2. The Vendor Payment modal's bank account picker rendered GL accounts (Bank Accounts parent, Primary Bank Account, Bank Charges) instead of HDFC Bank. User couldn't select a valid bank → save validation rejected the form → silent early-return in saveVendorPayment. **Tier 1 fix #2 was correct but the picker upstream of it was broken.** |
| B3 | Form-button submit-default protector | `js/accounts/accounts-common.js` — added a global DOMContentLoaded patch + MutationObserver that adds `type="button"` to every `<button>` inside a `<form>` that doesn't have an explicit type attribute | Hot-fix for the HTML default behavior — `<button>` inside a `<form>` defaults to `type="submit"`, which would submit the form and abort any in-flight async API call. Most accounts modals had this latent bug for buttons that weren't outside the form (the immediate cases I tested were outside the form so weren't affected, but this protects every other modal in the module from the same trap). |

### Tier 3 — Orphan-call backend endpoints (10/10 ✓ backend, 0/10 click-verified)

> Date: 2026-04-10. Each row corresponds to one of the 10 orphan frontend calls flagged in Phase 2.
> The backend endpoints are built and wired to BL methods. **No row is "shipped" until the click-test column is ✓** — per user direction, every fix must be walked through the real UI in Playwright (not just `browser_evaluate` API spies) so we catch modal/binding/refresh bugs that integration tests miss.

| # | Endpoint built | Controller : method | BL method | Backend ✓ | Click-test |
|---|---|---|---|---|---|
| 1 | `PUT /fiscal/years/{id}` | `FiscalController.UpdateFiscalYear` | `BL.UpdateFiscalYear` | ✓ rename-only by design (`UpdateFiscalYearRequest` has just `name`); 0-row → 404 if year is closed | pending |
| 2 | `PUT /invoices/{id}` | `CustomerInvoicesController.Update` | `BL.UpdateDraftInvoice` | ✓ accepts `CreateCustomerInvoiceRequest`; status check should reject non-draft | pending |
| 3 | `DELETE /vendor-bills/payments/{id}` | `VendorBillsController.DeleteVendorPayment` | `BL.DeleteVendorPayment` | ✓ soft-delete (sets `is_active=false`); should reverse allocations and bump bill `balance_due` back | pending |
| 4 | `DELETE /journals/types/{id}` | `JournalsController.DeleteJournalType` | `BL.DeleteJournalType` | ✓ 404 on system types | pending |
| 5 | `DELETE /expenses/policies/{id}` | `ExpenseController.DeletePolicy` | `BL.DeleteExpensePolicy` | ✓ 404 if not found | pending |
| 6 | `DELETE /expenses/categories/{id}` | `ExpenseController.DeleteCategory` | `BL.DeleteExpenseCategory` | ✓ 404 if not found | pending |
| 7 | `PUT /closing/checklists/{id}` | `ClosingController.UpdateChecklist` | `BL.UpdateClosingChecklist` | ✓ accepts `closing_type` only; 404 if completed | pending |
| 8 | `DELETE /closing/checklists/{id}` | `ClosingController.DeleteChecklist` | `BL.DeleteClosingChecklist` | ✓ 404 if not found or already completed | pending |
| 9 | `POST /audit/approvals/{id}/approve` | `AuditController.ApproveApproval` | proxy → `BL.ApproveExpenseClaim` | ✓ thin proxy; **caveat:** only expense claims surface in pending list today, future approval-gated entities (vendor bills, customer invoices, etc.) need their own dispatch logic before this endpoint generalizes | pending |
| 10 | `POST /audit/approvals/{id}/reject` | `AuditController.RejectApproval` | proxy → `BL.RejectExpenseClaim` | ✓ accepts `RejectApprovalBody { reason }`; same proxy caveat | pending |

**Open follow-ups discovered while logging Tier 3:**

1. **Frontend payload trims** — at least 2 of the new endpoints accept narrower DTOs than the frontend currently sends:
   - `PUT /fiscal/years/{id}` → frontend sends `{name, start_date, end_date}`, only `name` honored. Should trim or extend the model.
   - `PUT /closing/checklists/{id}` → frontend likely sends `{fiscal_period_id, closing_type}`, only `closing_type` honored. Same call.
2. **Audit "approvals" abstraction is leaky.** The proxy approve/reject endpoints assume the `id` is always an expense claim. If the pending list ever returns vendor bills or invoices (per backend stub note row 6 in the stub table), clicking Approve will silently update the wrong table or 404. Phase 4b should either generalize (add `entity_type` to the route) or rename to `expenses/...` to be honest.
3. **Vendor payment delete** needs to verify the BL actually reverses allocations. If `DeleteVendorPayment` only flips `is_active=false` without touching the allocation rows or the bill's `balance_due`, voiding a payment will leave the bill stuck in `partially_paid` status with a phantom paid_amount. **This is the most accounting-critical of the 10 — must be the first click-test.**
