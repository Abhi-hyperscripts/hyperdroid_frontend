...
| 4. Fix gaps | in-progress | **Tier 1 complete (5/5):** payment allocations (AR+AP), Save&Approve (AR+AP), Year-End preflight, dashboard pending approvals badge. Each verified end-to-end via Playwright with two-step proof. SW bumped 874→880. **Tier 3 complete on backend (10/10 endpoints, 2026-04-10):** all 10 orphan calls now have working backend routes. **Click-through verification pending for every row** — none can be marked "shipped" until Playwright walks the actual UI button → modal → save → list-refresh chain. |
